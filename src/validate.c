...
vst2name(int st)
{
	...
	case JVST_ST_FETCH_PROP:
		return "ST_FETCH_PROP";
	case JVST_ST_AFTER_VALUE:
		return "ST_AFTER_VALUE";
...
static void
dump_stack(struct jvst_validator *v)
{
	size_t i, n;
	n = v->stop;
	if (n > v->smax) {
	...
	for (i = 0; i < n; i++) {
		...
		fprintf(stderr, "[%zu] %-16s %8p %8zu", i, vst2name(sp->state), (void *)sp->schema,
			sp->nitems);
...
static void
copycounters(struct jvst_state *dst, const struct jvst_state *src)
{
	size_t i, n;
...
static void
zerocounters(struct jvst_state *sp)
{
	size_t i, nc, nw;
	sp->nitems = 0;
...
jvst_validate_init_defaults(struct jvst_validator *v, const struct ast_schema *schema)
{
	...
	(void)sjp_parser_init(&v->p, &v->pstack[0], ARRAYLEN(v->pstack), &v->pbuf[0],
			      ARRAYLEN(v->pbuf));
}

static inline struct jvst_state *
getstate(struct jvst_validator *v)
{
...
static inline struct jvst_state *
pushstate(struct jvst_validator *v, int st, const struct ast_schema *schema)
{
...
static inline struct jvst_state *
newframe(struct jvst_validator *v)
{
	return pushstate(v, JVST_ST_VALUE, NULL);
}

static inline struct jvst_state *
returnframe(struct jvst_validator *v)
{
	if (v->stop >= v->smax) {
		return NULL;
	}
...
static inline struct jvst_state *
popstate(struct jvst_validator *v)
{
	if (v->stop == 0 || v->stop > v->smax) {
		return NULL;
	}
...
static int
invalid(struct jvst_validator *v, const char *file, int line, const char *fmt, ...)
{
	if (v->stop < v->smax) {
...
eat_value(struct jvst_validator *v, enum SJP_RESULT pret, struct sjp_event *evt)
{
	struct jvst_state *sp;
...
static enum jvst_result
eat_array(struct jvst_validator *v, enum SJP_RESULT pret, struct sjp_event *evt)
{
	struct jvst_state *sp;
	...
	if (sp->counters[CI_A_NUM_OBJS] == 0 && sp->counters[CI_A_NUM_ARRS] == 0 &&
...
// in the nitems field of the return stack frame.
static enum jvst_result
eat_object(struct jvst_validator *v, enum SJP_RESULT pret, struct sjp_event *evt)
{
	struct jvst_state *sp;
...
static enum jvst_result
validate_object(struct jvst_validator *v, enum SJP_RESULT pret, struct sjp_event *evt);

static inline bool
is_valid_type(struct jvst_validator *v, enum json_valuetype mask, enum json_valuetype bits)
{
	...
	// something like: validate_set_error(v, "expected type mask %s but found type %s", mask,
	// bits);
	return false;
}

static enum jvst_result
validate_number(struct jvst_validator *v, enum SJP_RESULT pret, struct sjp_event *evt)
{
	struct jvst_state *sp;
	const struct ast_schema *schema;
	...
	if (schema->kws & KWS_MINIMUM) {
		if (schema->exclusive_minimum) {
			if (evt->d <= schema->minimum) {
				return INVALID(v, "invalid number: %f <= %f", evt->d,
					       schema->minimum);
			}
...
static enum jvst_result
validate_integer(struct jvst_validator *v, enum SJP_RESULT pret, struct sjp_event *evt)
{
	double x, cx;
...
static inline enum jvst_result
check_type_priv(struct jvst_validator *v, enum json_valuetype mask, enum json_valuetype bits,
...
static enum jvst_result
validate_next_event(struct jvst_validator *v, enum SJP_RESULT pret, struct sjp_event *evt);

static enum jvst_result
validate_some_of(struct jvst_validator *v, enum SJP_RESULT pret, struct sjp_event *evt)
{
	struct jvst_state *sp;
	const struct ast_schema *schema;
...
init:
	...
	if (vv == NULL) {
		return INVALID(v, "cannot allocate %zu split validators for some_of evaluation",
			       count);
	}
	sp->u.split.vv = vv;
	sp->u.split.nv = count;
...
static enum jvst_result
validate_value(struct jvst_validator *v, enum SJP_RESULT pret, struct sjp_event *evt)
{
	struct jvst_state *sp;
	...
	case SJP_NUMBER:
		...
		if ((sp->schema->types & (JSON_VALUE_NUMBER | JSON_VALUE_INTEGER)) ==
		    JSON_VALUE_INTEGER) {
...
static const struct ast_schema *
match_property(struct ast_property_schema *set, const char *k, size_t n)
{
...
static size_t
bit_from_set(const struct ast_string_set *ss, const char *k, size_t nk)
{
	size_t b = 1;
	...
	if (schema->required.set != NULL) {
		...
		for (ss = schema->required.set; ss != NULL; ss = ss->next) {
			...
			if (reqmask == full_mask) {
				...
				fprintf(stderr,
					"warning: only %zu different required properties are "
					"supported\n",
					nreq - 1);
			}
		}
...
check_obj:
	...
	if (venum & OBJ_MINMAX_PROPERTIES) {
		if (sp->nitems < schema->min_properties) {
			return INVALID(v, "object has %zu properties, min is %zu", sp->nitems,
				       schema->min_properties);
		}

		if (schema->max_properties > 0 && sp->nitems > schema->max_properties) {
			return INVALID(v, "object has %zu properties, max is %zu", sp->nitems,
...
validate_next_event(struct jvst_validator *v, enum SJP_RESULT pret, struct sjp_event *evt)
{
	...
	fprintf(stderr, "pret = %d (%s)  evt->type = %d (%s)  evt->text = '%s'\n", pret,
		ret2name(pret), evt->type, evt2name(evt->type), dbgbuf);
	dump_stack(v);
	...
	do {
		...
		fprintf(stderr,
			"--> %zu [%s] schema = %p  nitems = %zu  counters=[%zu %zu]  work=[%" PRIu64
			" %" PRIu64 "]\n",
			v->stop, vst2name(top->state), (void *)top->schema, top->nitems,
			top->counters[0], top->counters[1], top->u.work[0], top->u.work[1]);
